...
class FontFlip {
    constructor(el, options = {}) {
        this.el = el;
        this.letterSpacing = options.letterSpacing || '0.1em';
        this.fonts = [
    ...
    _renderWord(activeIndex, fontFrame, fontList = this.fonts) {
        ...
        for (let i = 0; i < this.text.length; i++) {
            ...
            //getBoundingClientRect can be slow, but necessary here for accuracy
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // This function runs after the main DOMContentLoaded event that sets up the rest of the page logic
    function initializeIntro() {
        // Check if intro has been shown before
        if (!sessionStorage.getItem('introShown')) {
            if (introOverlay && introText) {
                introOverlay.style.display = 'flex'; // Make sure it's visible
                const introFlip = new FontFlip(introText, { letterSpacing: '0.02em' });
    ...
    function startSubtitleAnimation(overlay, textElement) {
        // Prepare for subtitle
        textElement.style.fontSize = 'clamp(10.8px, 2.43vw, 29.7px)'; // Adjust font size for subtitle
        textElement.style.lineHeight = '1.4';
        textElement.style.whiteSpace = 'pre-wrap'; // Allow line breaks
        textElement.style.color = '#2d2f32'; // Subtitle color
        ...
        function showNextParagraph() {
            if (paragraphIndex < paragraphs.length) {
                ...
                paragraphIndex++;
                setTimeout(showNextParagraph, 1500); // Reduced delay
            } else {
    ...
    // Ensure the rest of the DOM is ready, then run the intro
    setTimeout(initializeIntro, 0);
});
